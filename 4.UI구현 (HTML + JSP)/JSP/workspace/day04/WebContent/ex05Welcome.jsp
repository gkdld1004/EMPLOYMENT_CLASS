...
<p>니가 입력한 ID : <%=request.getParameter("userid") %></p>
<p>니가 입력한 PW : <%=request.getParameter("userpw") %></p>
<%
	Login result = (Login)request.getAttribute("result");
	// getAttribute() 의 반환형은 Object이므로, 적절한 다운캐스팅이 필요하다
%>
<p>로그인한 계정의 사용자 이름 : <%=result.getUsername() %></p>
<p>로그인한 계정의 사용자 이름 : 
	<%=((Login)request.getAttribute("result")).getUsername() %>
</p>
<p>EL Tag로 출력한 값 : ${result.username }</p>
<%--
		사용자의 입력데이터(문자열) -> 서버
