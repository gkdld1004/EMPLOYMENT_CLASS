...
<%!
	Login[] arr = {
			...
			new Login("itbank", "it", "아이티뱅크"),
	};

	public Login login(Login param) {
		for(int i = 0; i < arr.length; i++) {
			if(param.getUserid().equals(arr[i].getUserid())) {		// ID
				if(param.getUserpw().equals(arr[i].getUserpw())) {	// PW
...
<%
	...
	if(result != null) {
		// 결과가 있으면 환영 메시지를 출력하는 Welcome.jsp의 내용을 보여줘라(주소 변경 없음, 요청 유지됨)
		String path = "ex05Welcome.jsp";
		RequestDispatcher rd = request.getRequestDispatcher(path);
		
		// forward하기 전에 request객체에다가 login메서드의 반환 객체 result를 추가해두면
		// Welcome 페이지에서 객체에 접근하여 이름을 출력할 수 있다
