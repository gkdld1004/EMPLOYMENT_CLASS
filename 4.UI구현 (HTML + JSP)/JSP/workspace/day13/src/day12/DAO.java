...
public class DAO {
	...
	public static DAO getInstance() {	// 외부에서는 getInstance를 호출하여 이미 생성된 객체 하나를 반환받는다
		return instance;	// 일반 메서드는 객체 생성 이후 메서드를 호출할 수 있으나
	}						// 생성자를 접근제한자로 막았기 때문에, 객체를 통한 접근은 불가능하다 (클래스 메서드로 접근가능하도록)
	
	private DAO() {	// 생성자가 private이므로 외부에서는 더이상 새로운 객체를 생성할 수 없게 된다
		try {
	...
	public String test() {
		...
		try {
			...
			System.out.println("SQL Exception(test) : " + e);
			e.printStackTrace();
