...
<c:set var="map" value="${boardDAO.selectAll(param.page, param.type, param.word) }" />
...
<table style="border:1px solid black;">
	...
	<c:forEach var="dto" items="${map.list }">
		<tr>
			<td>${dto.idx }</td>
			<td><a href="${cpath }/view.jsp?idx=${dto.idx}&page=${param.page}&type=${param.type }&word=${param.word }&c=plus">${dto.title }</a></td>
			<td>${dto.username }</td>
			<td>${dto.viewCount }</td>
			<td>${dto.ipaddr }</td>
		</tr>
