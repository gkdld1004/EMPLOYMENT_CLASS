...
<%--
	...
	1) directive, 두 파일을 컴파일 전에 미리 합쳐서, 합쳐진 하나의 파일을 컴파일한다 (지역 공유)
	   header가 가지는 변수를 다른 파일들에서 공유하고 싶을 때
	   
	2) action tag, 각각의 파일을 컴파일 하고, 나온 결과물(HTML 태그)을 합쳐서 보여준다 (지역 공유 안함)
	      상단과 하단의 지역 변수 충돌이 없도록 하고 싶을 때

--%>
<jsp:include page="header.jsp" />	<%-- 지역변수 test가 선언되어 있음 --%>
<%
	String test = "ex01";
%>
<h2>test : <%=test %></h2>
...
<%--
<jsp:useBean id="bean" class="java.util.ArrayList" /><!--객체 생성 -->
