...
<body>
<%!
	// 회원정보에 이름이 없으면 좀 별로니까, 이름도 넣어야 하는데
	// 입력에는 이름이 없으니까, 자바 빈즈를 써야겠다 -> Member.java
	
	Member[] arr = {	// 생성자 오버로딩 했으니, 객체를 배열로 만들어야지 (리스트로 할까? 귀찮음)
		new Member("test", "1234", "테스트"),	
		new Member("root", "1", "관리자"),	
		new Member("itbank", "it", "아이티뱅크"),	
		new Member("iu930516", "iu", "이지은"),	
	};
	// 만약 회원 가입 기능을 구현해야 했다면 이렇게 썼을 것
// 	ArrayList<Member> list = new ArrayList<Member>(Arrays.asList(arr));
%>
<%	request.setCharacterEncoding("UTF-8");	// 빈즈로 입력받을껀데, POST니깐 인코딩 설정해야지 %>
<jsp:useBean id="login" class="day06.Member" />
...
<%
	...
	Cookie idcookie = new Cookie("userid", login.getUserid());	// 쿠키를 만들고 ID를 담는다
