...
<%--
		...
		4) 다음번에 사이트에 다시 방문시, 전달받았던 쿠키를 서버에 같이 보낸다
...
<%
	...
	// 사이트 첫 방문시에는 쿠키가 없다 (쿠키는 재 방문시에 활용되는 값이다)
	...
	for(int i = 0; cs != null && i < cs.length; i++) {	// 쿠키 배열 중에서
		...
		if(c.getName().equals("comment")) {	// c의 이름이 comment이면
			...
			autoComplete = autoComplete.replace("_", " ");	
											// 언더바 문자를 띄워쓰기 공백으로 치환해라
		}
	}
%>
...
<form action="ex04Result.jsp">
	<input type="text" name="comment" value="<%=autoComplete %>" 
