...
class Dog extends Animal {
	@Override
	void speak() {		// 상속받은 메서드를 재정의 (오버라이딩)
		System.out.println("멍멍");
	}

	void tailing() {		// 상속받지 않은 기능, Dog의 고유 기능
...
class Cat extends Animal {
	...
	void scratch() {		// 고유 메서드
		System.out.println(name + "가 벽을 긁는다.");
	}
...
//		ob1.tailing();		// Animal에는 정의되어 있지 않은 메서드
		...
		// 오버라이딩 메서드는 자식 객체를 부모클래스로 참조해도 자식클래스의 메서드가 호출된다.
		
		Animal ob2 = c1;		// 자식 객체를 부모 타입으로 참조하기 위한 형 변환 (업 캐스팅) 
								// 암묵적 형변환 (여러 객체를 한번에 제어할 수 있다.)
		
		ob2.speak();
//		ob2.scratch();		// Animal에는 정의되어 있지 않은 메서드
		...
		for(int i = 0; i < arr.length; i++) {
			arr[i].speak();
			if(arr[i] instanceof Dog) {		// 자료형 상관없이 객체가 Dog 타입이면
				((Dog)arr[i]).tailing();	// 객체를 Dog로 형 변환
			}
			if(arr[i] instanceof Cat) {		
				((Cat)arr[i]).scratch();	// 업 캐스팅된 객체를 원래 자식 타입으로 형변환
