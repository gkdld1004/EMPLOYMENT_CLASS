...
public class Ex05_Client {
	public static void main(String[] args) throws Exception {
		...
		// 127.0.0.1 은 어느 상황에서나 자기 자신의 컴퓨터를 가리키는 IP주소이다.
		
		OutputStream os = so.getOutputStream();		// 지정한 소켓을 대상으로 출력하는 스트림
		OutputStreamWriter osw = new OutputStreamWriter(os);	// 글자를 출력하는 스트림
		...
		pw.close();		bw.close();		osw.close();
		os.close();		so.close();		sc.close();
	}
}
