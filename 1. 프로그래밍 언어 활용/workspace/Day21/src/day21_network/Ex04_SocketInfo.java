...
public class Ex04_SocketInfo {
	...
	/*
	 ...
	 Node 중에서 IP주소를 가지는 End Device(말단 장비)를 Host라고 한다.
	 (Router, Switch, Hub 등의 장비는 Host에서 제외)
	  
	 Host 내부의 실행중인 프로그램을 Process라고 한다.
	  하나의 Host(컴퓨터 or 스마트폰)가 여러 개의 Process를 구동할 수 있으며
	  데이터를 주고 받는 주체는 Process 이다.
	 ...
	 Host 의 특정 Process 를 찾아갈 수 있어야 한다.
	 ...
	 두개의 프로그램을 동시에 구동하여 웹서버를 구현하려면 서로 다른 포트를 이용해야 한다.
	 80번 포트 개방은 하나만 가능하다.)
