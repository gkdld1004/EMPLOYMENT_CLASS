...
class Input implements Runnable {
	...
	Input(Socket so){		// Input 객체를 생성하기 위해서는 대상의 정보를 가지고 있는 Socket 객체를 전달해야만 한다.
		this.so = so;
	}
...
public class Ex06_Server {
	public static void main(String[] args) {
		...
		try {
			...
			Thread th = new Thread(input);		// Input 객체를 Thread화 한다.
			th.start(); 	 					// 입력받아서 소켓대상에게 문자열을 보내는 스레드
			
			while(true) {
				msg = sc.nextLine();			// sc로 대상이 보낸 메시지를 받아서
