...
public class Server {
	...
	static void sendMessage(String str) {		// 접속된 모든 클라이언트에게 데이터를 출력 (보내기)
		if(str == null) return;
	...
	public static void main(String[] args) {
		...
		try {
			...
			while(true) {
				System.out.printf("접속 대기중...(%d)", plist.size() - 1);
				so = ss.accept();
...
//				sendMessage("\n\t" + host + "님이 접속했습니다.\n");
				...
				new Thread(ct).start();
				System.out.println(host + "님에게 보낼 수 있는 스트림을 plist에 추가합니다.");
				plist.add(pw);
			}
