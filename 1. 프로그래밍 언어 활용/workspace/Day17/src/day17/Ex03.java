...
//			al1.get(i).show();
//			객체를 ArrayList에 넣으면, Object 타입으로 취급된다.
//			Object 클래스에는 show()메서드가 정의되어 있지 않다 !! => 호출 불가
...
//			1) 다운캐스팅, 강제 형변
		...
		for(int i = 0; i < al2.size(); i++) {
			al2.get(i).show();
			// 내부의 요소(객체)들이 자료형 Member로 취급되고 있다 -> 형변환이 필요없다.
		}
		al2.get(3).show();
//		al2.get(3).test();
//		제네릭 타입이 Member인데 Member에는 test()메서드가 선언되어 있지 않으므로 호출 불가
		VIP vip = (VIP)al2.get(3);
		vip.test();
