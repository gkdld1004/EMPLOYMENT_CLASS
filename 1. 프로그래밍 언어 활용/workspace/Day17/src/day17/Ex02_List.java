...
//		List list1 = new List();
		// Cannot instantiate the type List : List 타입은 객체화 할 수 없습니다.
		
		int[] arr = new int[5];
		ArrayList al = new ArrayList();
		
		arr[0] = 2;		// 배열의 특정 순번위치에 값을 저장
		arr[1] = 7;		// 배열이 크기만큼만 데이터를 저장할 수 있다.
		...
		al.add("ITBANK");		// 문자열, String
		al.add(3.14);			// 실수, double
		al.add(7);				// 이미 들어가 있는 값을 추가함
		...
		al.remove(2);		// (0부터 시작해서) 2번째에 있는 값을 제거 후 값을 한칸씩 앞당긴다.
		...
		System.out.println("3.14는 몇번째에 위치하는가 : " + al.indexOf(3.14));
		System.out.println("3.15는 몇번째에 위치하는가 : " + al.indexOf(3.15));		// 없는 값을 넣으면 -1을 반환
		
		al.clear();		// 리스트 내부의 모든 값을 삭제하고, 크기를 0으로 되돌린다. (리스트 초기화)
