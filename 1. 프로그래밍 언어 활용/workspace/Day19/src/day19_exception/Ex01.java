...
public class Ex01 {
	...
	// Bug			(버그) : 코드 상에는 문제가 없으나, 간헐적으로 발생하는 문제
...
//		input = sc.nextLine();		// 입력 내용을 문자열로 입력받고
//		
...
//		catch	: try에서 예외 객체가 발생하면 catch에서 받아서 적절한 처리코드를 넣어준다.
//		throw	: 예외 객체를 생성해서 강제로 catch에게 전달한다.
//		throws	: 예외가 발생하면 함수에서 자신을 호출한 caller에게 예외 처리를 전가한다.
//		final	: 예외 발생 여부에 상관없이 마지막으로 반드시 실행해야 할 코드를 작성한다. (주로 객체 종료 코드)
