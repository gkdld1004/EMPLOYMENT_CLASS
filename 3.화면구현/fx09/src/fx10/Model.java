...
public class Model {
	...
	private final String fileName = "phonebook-fx10.dat";
	...
	void save() throws IOException {	// 파일에 저장
		File f = new File(fileName);						// 파일 객체
	...
	void load() throws IOException, ClassNotFoundException {
		...
		if(f.exists() == false) {
			System.out.println("저장된 파일이 없습니다 : " + fileName);
			f.createNewFile();
		...
		list = FXCollections.observableList(load); 
		for(MemberVO m : list) {
			m.setImage(new ImageView(getClass().getResource(m.getImage_s()).toString()));
