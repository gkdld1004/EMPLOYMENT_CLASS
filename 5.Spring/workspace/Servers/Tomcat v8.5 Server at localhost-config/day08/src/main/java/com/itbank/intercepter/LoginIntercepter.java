...
public class LoginIntercepter extends HandlerInterceptorAdapter {
	...
	// 요청을 중간에 가로채서 먼저 처리할 내용을 처리하고, 이후 컨트롤러의 메서드로 연결할지 말지 결정할 수 있다 (preHandler)
		
	// 인터셉터는 클래스 상속으로 만들 수 있다
	// extends 는 implements와 달리 오버라이딩을 강제하지 않기 때문에 메서드가 자동으로 만들어지지는 않는다
	// 이클립스의 Override / Implements Method 메뉴를 이용하면 편리하게 작성 가능
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		// substring 은 문자열에서 특정 인덱스를 이용해 문자열을 잘라내는 문자열 뺄셈의 기능이다
		//  0123456
		// [/day08/gallery]
...
//		response.sendRedirect(request.getContextPath() + "/login");		// ${cpath}/login
